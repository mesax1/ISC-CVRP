...
std::vector<std::vector<float>> DecisionTree::PredictClassProbabilities(
  ...
  std::vector<float> current_class_probabilities;
  ...
  for (std::size_t n_sample = 0; n_sample < leaf_node_ids.size(); n_sample++) {
    ...
    for (std::size_t n_class = 0; n_class < nodes[0].value.size(); n_class++) {
      current_class_probabilities.push_back(
          ...
          static_cast<float>(nodes[current_node_id].n_samples));
    }
...
std::vector<int> DecisionTree::PredictClasses(
    const std::vector<std::vector<float>> *feature_data_ptr) {
  if (!is_built) {
    throw std::logic_error(
        "Decision tree must be built with BuildTree method before predictions "
        "can be made.");
...
DecisionTreeClassifier::DecisionTreeClassifier(
    ...
    MaxFeaturesMethod max_features_method, float min_impurity_split) {
