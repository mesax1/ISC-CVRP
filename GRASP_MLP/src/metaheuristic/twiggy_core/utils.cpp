...
void Split_X_y(table &data_frame, std::vector<std::vector<float>> &X,
...
void table::AddRow(std::vector<float> row) { data_.push_back(row); }
...
int ArgMax(std::vector<float> &vector) {
  std::size_t i = 0;
