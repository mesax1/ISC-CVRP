...
float Criterion::ImpurityImprovement() {
  float impurity_left;
...
float Criterion::GiniCoefficient(std::vector<int> &label_freqs, int &n_samples,
                                  int &n_labels) {
  float freq_squares = 0.0;
...
float Criterion::Entropy(std::vector<int> &label_freqs, int &n_samples,
                          int &n_labels) {
  float entropy = 0.0;
  float label_frequency = 0.0;
...
void Criterion::ChildrenImpurities(float &impurity_left,
                                   float &impurity_right) {
