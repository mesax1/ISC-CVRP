...
struct DecisionTree {
  ...
  std::vector<std::vector<float>> PredictClassProbabilities(
...
class DecisionTreeClassifier {
 public:
  ...
  DecisionTreeClassifier(ImpurityMeasure impurity_measure = gini,
                         ...
                         float min_impurity_split_ = 0.0);

  void BuildTree(const std::vector<std::vector<float>> *feature_data_ptr,
                 const std::vector<int> *label_data_ptr,
                 const std::vector<int> samples_subset, int n_labels,
                 std::mt19937 *gen_);

  void BuildTree(const std::vector<std::vector<float>> feature_data,
