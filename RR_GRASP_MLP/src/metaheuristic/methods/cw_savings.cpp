...
shared_ptr <Solution> ParallelSavings::clarke_and_wright(const Instance& instance, shared_ptr <Solution> solution, float lambda, int neighbors_num, int beta) {
    ...
    */
    ...
    rcl_list.reserve(static_cast<unsigned long>(10));
    for (int i = 0; i < 10 ; i++)
    {
        rcl_list.push_back(savings[i]);
    }
    for (int i = 10; i < savings.size() ; i++)
    {
        selected_candidate = rand() % 10;
