...
DTOP_values dataset_determine_client_pairs_parameters(vector<SampleInfo*>& dataset_train, Instance& instance){
...
DDOP_values dataset_determine_vrp_parameters(vector<SampleInfo*>& dataset_train, Instance& instance){
...
DTOP_values dataset_determine_tsp_parameters(vector<SampleInfo*>& dataset_train, Instance& instance, int N=5){
...
DTOP_values dataset_to_vrp_parameters(vector<SampleInfo*>& dataset_train, Instance& instance){
	...
	for(int i = 0; i < dataset_train.size(); i++){
		test_x.push_back({s1_solution_intersections[i], s2_longest_distance_route[i], s3_average_depot_distance_route[i], s4_average_gravity_distance[i], 
	...
	test_x_names = {"S1", "S2", "S3", "S4", "S5", "S6", "S7",
	...
	DTOP_values parameters_and_names = {test_x, test_x_names};
	return parameters_and_names;
}

DTOP_values dataset_determine_tsp_vrp_parameters(vector<SampleInfo*>& dataset_train, Instance& instance, int N=5){
	...
	for(int i = 0; i < tsp_values.test_x.size(); i++){
		tsp_values.test_x[i].insert(tsp_values.test_x[i].end(), vrp_values.test_x[i].begin(), vrp_values.test_x[i].end());
	}
	
	tsp_values.test_x_names.insert(tsp_values.test_x_names.end(), vrp_values.test_x_names.begin(), vrp_values.test_x_names.end());
	
	return tsp_values;
}

DTOP_values dataset_determine_tsp_client_pairs_parameters(vector<SampleInfo*>& dataset_train, Instance& instance, int N=5){
	DTOP_values tsp_values = dataset_determine_tsp_parameters(dataset_train, instance, N);
	DTOP_values client_pairs_values = dataset_determine_client_pairs_parameters(dataset_train, instance);
	
	for(int i = 0; i < tsp_values.test_x.size(); i++){
		tsp_values.test_x[i].insert(tsp_values.test_x[i].end(), client_pairs_values.test_x[i].begin(), client_pairs_values.test_x[i].end());
	}
	
	tsp_values.test_x_names.insert(tsp_values.test_x_names.end(), client_pairs_values.test_x_names.begin(), client_pairs_values.test_x_names.end());
	
	return tsp_values;
}

DTOP_values dataset_determine_vrp_client_pairs_parameters(vector<SampleInfo*>& dataset_train, Instance& instance){
	DTOP_values vrp_values = dataset_to_vrp_parameters(dataset_train, instance);
	DTOP_values client_pairs_values = dataset_determine_client_pairs_parameters(dataset_train, instance);
	
	for(int i = 0; i < vrp_values.test_x.size(); i++){
		vrp_values.test_x[i].insert(vrp_values.test_x[i].end(), client_pairs_values.test_x[i].begin(), client_pairs_values.test_x[i].end());
	}
	
	vrp_values.test_x_names.insert(vrp_values.test_x_names.end(), client_pairs_values.test_x_names.begin(), client_pairs_values.test_x_names.end());
	
	return vrp_values;
}

DTOP_values dataset_determine_tsp_vrp_client_pairs_parameters(vector<SampleInfo*>& dataset_train, Instance& instance, int N=5){
	DTOP_values tsp_values = dataset_determine_tsp_parameters(dataset_train, instance, N);
	DTOP_values vrp_values = dataset_to_vrp_parameters(dataset_train, instance);
	DTOP_values client_pairs_values = dataset_determine_client_pairs_parameters(dataset_train, instance);
...
STOP_values solution_to_other_parameters(std::shared_ptr <Solution> initial_solution, Instance& instance){
	...
	test_x = {s1_solution_intersections[i], s2_longest_distance_route[i], s3_average_depot_distance_route[i], s4_average_gravity_distance[i], 
						s5_average_width_route[i], s6_average_span_route[i], s7_average_width_compactness[i], s8_average_span_compactness[i], 
						s9_average_depth_route[i], s10_std_dev_customers_route[i], s21[i], s22[i], s23[i], s24[i],
						s31[i], s32[i], s51[i], s52[i], s61[i], s62[i], s71[i], s72[i], 
						s81[i], s82[i], s91[i], s92[i], 
	...
	STOP_values parameters_and_names = {test_x, test_x_names};
	return parameters_and_names;
}

void print_features(vector<SampleInfo*>& dataset_train, Instance& instance, string& method, string& file_parameters, string& direc_output){
	DTOP_values values;
	string filename;
	if(method == "tsp"){
	...
	for(int i = 0; i < values.test_x_names.size() - 1; i++){
		file << values.test_x_names[i] << ",";
	}
	
	file << values.test_x_names[values.test_x_names.size() - 1] << ",";
	file << "final_cost" << endl;  //Add final cost ONLY FOR TRAINING SET
	
	for(int i = 0; i < dataset_train.size(); i++){
		file << i << ",";
		for(int j = 0; j < values.test_x[i].size() - 1; j++){
			file << values.test_x[i][j] << ",";
		}
		file << values.test_x[i][values.test_x[i].size() - 1] << ",";
...
void print_vnd_features(vector<SampleInfo*>& dataset_train, Instance& instance, string& method, string& file_parameters, string& direc_output, int number_of_vnds){
    DTOP_values values;
    string filename;
    if(method == "tsp"){
        cout << "Entra a TSP" << endl;
...
void print_validation_features(vector<SampleInfo*>& dataset_train, Instance& instance, string& method, string& file_parameters, string& direc_output){
	DTOP_values values;
	string filename;
	...
	file << values.test_x_names[values.test_x_names.size() - 1] << endl;
	
	
	for(int i = 0; i < dataset_train.size(); i++){
		file << i << ",";
		for(int j = 0; j < values.test_x[0].size() - 1; j++){
			file << values.test_x[i][j] << ",";
		}
		file << values.test_x[i][values.test_x[0].size() - 1] << endl;
	}
